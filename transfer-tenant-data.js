...
return (
    initConnection(sourceDatabase)
        ...
        .then(() => {
            ...
            if (_.isEmpty(result)) {
                logger.info(
                    `${chalk.green(`✓`)}  No Discussions rows found...`
                );
        ...
        .then(result => {
            ...
            logger.info(
                `${chalk.green(
                    `✓`
                )}  Inserting MessageBoxRecentContributions...`
            );
            return data.targetClient.batch(allInserts, { prepare: true });
        })
        .then(() => {
        ...
        .then(result => {
            if (_.isEmpty(result.rows)) {
                logger.info(
                    `${chalk.green(
                        `✓`
                    )}  No FollowingUsersFollowers rows found...`
                );
                return;
            }
        ...
        .then(result => {
            if (_.isEmpty(result.rows)) {
                logger.info(
                    `${chalk.green(
                        `✓`
                    )}  No FollowingUsersFollowing rows found...`
                );
                return;
            }
        ...
        .then(result => {
            if (_.isEmpty(result.rows)) {
                logger.info(
                    `${chalk.green(
                        `✓`
                    )}  No AuthenticationLoginId rows found...`
                );
                return;
            }
        ...
        .then(result => {
            if (_.isEmpty(result.rows)) {
                logger.info(
                    `${chalk.green(`✓`)}  No OAuthClient rows found...`
                );
                return;
            }
        ...
        .then(result => {
            ...
            logger.info(
                `${chalk.green(
                    `✓`
                )}  Inserting AuthzInvitationsTokenIdByEmail...`
            );
            return data.targetClient.batch(allInserts, { prepare: true });
        })
        .then(() => {
